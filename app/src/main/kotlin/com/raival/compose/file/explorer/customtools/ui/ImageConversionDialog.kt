...
class ImageConversionDialog : DialogFragment() {
    ...
    companion object {
        ...
        fun showPdf(context: Context, paths: List<String>) {
            val valid = paths.filter {
                val file = java.io.File(it)
                file.exists() && file.isFile && file.extension.equals("pdf", true)
            }
            if (valid.isNotEmpty()) {
                PdfPageExtractionDialog.show(context, valid)
            }
        }

        fun showImage(context: Context, paths: List<String>) {
            val valid = paths.filter {
                val file = java.io.File(it)
                file.exists() && file.isFile && file.extension.lowercase() in setOf("jpg", "jpeg", "png", "webp")
            }
            if (valid.isNotEmpty()) {
                show(context, valid.first())
            }
        }
    }
